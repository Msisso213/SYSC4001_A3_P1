...
struct PCB{
    ...
    unsigned int    io_time_elapsed;      // Tracks how long current I/O has been running
    unsigned int    cpu_time_since_io;    // Tracks CPU time since last I/O request
};
...
PCB add_process(std::vector<std::string> tokens) {
    ...
    process.io_time_elapsed = 0;
    process.cpu_time_since_io = 0;
